...
describe("외부 질의 30개 — 검색 품질 바닥선", () => {
   ...
   *
   * 대신 **개수 대신 구성을 못박는다**(아래 「5위 밖은 #18·#21 둘뿐이다」). 개수만 지키면
   * 다른 질의가 대신 떨어져도 28이라 초록이다. 어느 질의가 떨어졌는지를 재면 그 바꿔치기가
   * 붉게 나온다 — 자를 한 칸 내린 대가로 눈금을 더 촘촘하게 만든 것이다.
   */
