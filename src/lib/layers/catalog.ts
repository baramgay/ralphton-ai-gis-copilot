...
export const SKT_LIVING_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["이동통신 생활인구 (행정동, 시간대별 추정치의 월별 일평균)"],
  metrics: [
    { key: "living_total", label: "총생활인구", unit: "명", aggregation: "sum", formula: "월 전체 시간대 평균 생활인구", limitation: "이동통신 추정치, 실거주와 다를 수 있음", triggers: ["생활인구", "유동인구", "활동인구", "체류인구", "머무는"] },
    { key: "elderly_ratio", label: "생활인구 고령비중", unit: "%", aggregation: "weightedAvg", weightKey: "living_total", formula: "65세 이상 생활인구/총생활인구×100", limitation: "이동통신 추정치", triggers: ["생활인구 고령", "고령 생활", "생활인구 고령비중", "체류 고령"] },
  ],
};
...
export const SKT_MOBILITY_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  metrics: [
    { key: "inflow_total", label: "유입인구", unit: "명", aggregation: "sum", formula: "타 지역에서 유입된 일평균 생활인구", limitation: "이동통신 추정치, 통근·방문 등 일시 체류 포함", triggers: ["유입인구", "유입 인구", "들어오는", "유입되는", "유입"] },
    { key: "outflow_total", label: "유출인구", unit: "명", aggregation: "sum", formula: "타 지역으로 유출된 일평균 거주자", limitation: "이동통신 추정치", triggers: ["유출인구", "유출 인구", "빠져나가는", "빠져나가", "유출"] },
    { key: "net_flow", label: "순유입(유입−유출)", unit: "명", aggregation: "sum", formula: "유입인구 − 유출인구", limitation: "양수=순유입, 음수=순유출. 이동통신 추정치", triggers: ["순유입 인구", "순유입인구", "순유입", "순이동", "순인구이동"] },
  ],
};
...
export const SKT_DAYNIGHT_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["이동통신 생활인구 시간대 분해 (주간 09~18시 · 야간 22~05시 시간당 평균)"],
  metrics: [
    { key: "day_population", label: "주간인구", unit: "명", aggregation: "sum", formula: "09~18시 시간당 평균 생활인구", limitation: "이동통신 추정치, 통근·방문 등 일시 체류 포함", triggers: ["주간인구", "낮 인구", "낮인구", "주간 생활인구", "낮에 사람 많", "낮에만 사람"] },
    { key: "night_population", label: "야간인구", unit: "명", aggregation: "sum", formula: "22~05시 시간당 평균 생활인구", limitation: "이동통신 추정치, 실거주 인구와 다를 수 있음", triggers: ["야간인구", "밤 인구", "밤인구", "야간 생활인구", "심야인구", "밤에 사람 많"] },
...
export const NH_CONSUMPTION_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["카드 가맹점 유입지별 카드매출 (행정동, 전체카드 전수화 추정)"],
  metrics: [
...
export const NH_DEMOGRAPHICS_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["카드 가맹점 성연령별 카드매출 (행정동, 전체카드 금액 구성비)"],
  metrics: [
...
export const NH_HOURLY_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["카드 가맹점 시간대별 카드매출 (주간 09~18시 · 야간 22~05시, 주야간인구와 동일 구간)"],
  metrics: [
...
export const NH_INDUSTRY_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["카드 가맹점 업종별 카드매출 (표준산업분류 11차 대분류 기준 매출 비중)"],
  metrics: [
    ...
    { key: "education_share", label: "교육 소비비중", unit: "%", aggregation: "weightedAvg", weightKey: "card_sales", formula: "교육 서비스업(P) 매출 ÷ 전체 카드매출 × 100", limitation: "학원·교습 결제 중심이며 공교육 지출은 포함되지 않는다", triggers: ["교육 소비", "학원 소비", "사교육 소비", "교육업", "교육비 비중", "학원가", "학원 밀집"] },
  ],
};
...
export const NH_STORETYPE_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["카드 가맹점 업태별 카드매출 (표준산업분류 11차 소분류 기준 매출 비중)"],
  metrics: [
    ...
    { key: "pub_share", label: "주점 비중", unit: "%", aggregation: "weightedAvg", weightKey: "card_sales", formula: "일반·유흥 주점업 매출 ÷ 전체 카드매출 × 100", limitation: "가맹점 소재지 기준. 야간 상권 성격을 보조 설명", triggers: ["주점 비중", "술집 비중", "유흥 상권", "유흥업", "술집", "주점"] },
    ...
    { key: "medical_store_share", label: "병의원·약국 비중", unit: "%", aggregation: "weightedAvg", weightKey: "card_sales", formula: "종합병원·병의원·약국 매출 ÷ 전체 카드매출 × 100", limitation: "결제 기준이라 의료기관 수(공공 의료 레이어)와는 다른 관점", triggers: ["병의원 비중", "약국 비중", "약국 없는", "약국 적은", "약국 많은", "의료 업태"] },
  ],
};
...
export const KCB_CREDIT_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["신용정보 행정동 통계 (거주자 기준, 연령구간 인구가중 집계)"],
  metrics: [
    { key: "avg_income", label: "평균소득", unit: "만원/월", aggregation: "weightedAvg", weightKey: "pop_total", formula: "연령구간 월소득 평균의 인구가중 평균", limitation: "추정 거주자 소득, 세전 개인 기준", triggers: ["평균소득", "소득수준", "월소득", "부자", "잘 사는", "잘사는", "가난한", "못 사는", "소득"] },
    { key: "credit_score", label: "신용평점", unit: "점", aggregation: "weightedAvg", weightKey: "pop_total", formula: "신용평점(0~1000) 인구가중 평균", limitation: "신용정보 평점 기준", triggers: ["신용평점", "신용점수", "신용도"] },
    { key: "card_spend", label: "1인 카드소비", unit: "만원/월", aggregation: "weightedAvg", weightKey: "pop_total", formula: "카드 총이용금액 합 ÷ 소비활동 대상자 수", limitation: "거주자 기준이라 가맹점 상권 매출과 다르며, 신용판매·현금서비스·할부를 모두 포함한 카드 이용액이라 생활 소비만은 아니다", triggers: ["1인 카드소비", "인당 카드소비", "1인당 카드소비", "1인 소비", "인당 소비", "개인 소비", "1인소비"] },
...
export const KCB_MIGRATION_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["전입·전출 통계 (분기, 2년 전 거주지 대비 시군구 경계를 넘은 이동만)"],
  metrics: [
    { key: "move_in", label: "전입인구", unit: "명", aggregation: "sum", formula: "2년 전 거주 시군구가 다른 전입자 수(행정동 기준)", limitation: "같은 시군구 내 이동·미이동자는 제외. 일시 체류 유입인구와 달리 실제 거주지 이전", triggers: ["전입", "전입인구", "이사 온", "이주해 온"] },
...
export const KCB_COMMUTE_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  coverage: "gyeongnam",
  adminLevels: ["dong", "sgg"],
  sourceNotes: ["통근통계 (분기, 자택-직장 행정동 기준)"],
  metrics: [
    { key: "jobs_in", label: "일자리 유입", unit: "명", aggregation: "sum", formula: "그 행정동으로 출근하는 취업자 수(거주지 무관)", limitation: "직장 위치는 본사 주소로 잡히는 경우가 있어 실제 근무지와 다를 수 있다", triggers: ["일자리", "일자리 유입", "직장 인구", "출근 인구", "종사자"] },
...
export const KCB_GRID_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  sourceNotes: [
    ...
    "(격자×5세연령구간) 인구 3명 미만은 제공되지 않아 농촌이 크게 누락된다. 격자 표본이 행정동 인구의 90% 이상인 도시부(12개 시군구·95개 행정동)만 싣는다",
    "성인 30명 미만 칸은 평균·비율을 내지 않는다(표본 부족·재식별 방지)",
  ],
  metrics: [
    { key: "pop_total", label: "격자 성인인구", unit: "명", aggregation: "sum", formula: "만18~104세 집계인구 합", limitation: "3명 미만 연령구간이 빠져 실제 인구보다 적다. 도시부에서도 과소 추정이므로 절대 인구로 읽지 말 것", triggers: ["격자 인구", "격자 성인인구"] },
    ...
    { key: "card_spend", label: "격자 1인 카드소비", unit: "만원/월", aggregation: "weightedAvg", weightKey: "pop_total", formula: "카드 총이용금액 합 ÷ 소비활동 대상자 수", limitation: "성인 30명 이상 칸만 산출. 거주자 기준이라 가맹점 상권 매출과 다르다", triggers: ["격자 1인 카드소비", "격자 카드소비", "격자 소비"] },
