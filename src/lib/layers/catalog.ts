...
export const SKT_MOBILITY_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  metrics: [
    { key: "inflow_total", label: "유입인구", unit: "명", aggregation: "sum", formula: "타 지역에서 유입된 일평균 생활인구", limitation: "SKT 추정치, 통근·방문 등 일시 체류 포함", triggers: ["유입인구", "유입 인구", "유입"] },
    { key: "outflow_total", label: "유출인구", unit: "명", aggregation: "sum", formula: "타 지역으로 유출된 일평균 거주자", limitation: "SKT 추정치", triggers: ["유출인구", "유출 인구", "유출"] },
    { key: "net_flow", label: "순유입(유입−유출)", unit: "명", aggregation: "sum", formula: "유입인구 − 유출인구", limitation: "양수=순유입, 음수=순유출. SKT 추정치", triggers: ["순유입", "순이동", "순인구이동", "순유입인구"] },
  ],
};
...
export const NH_CONSUMPTION_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  metrics: [
    { key: "card_sales", label: "카드매출", unit: "백만원", aggregation: "sum", formula: "전체카드 이용금액 월 합계(전수화)", limitation: "가맹점 소재지 기준 상권 매출, 거주자 소비와 다름", triggers: ["카드매출", "소비매출", "상권매출", "카드소비", "매출"] },
    { key: "card_txns", label: "카드결제건수", unit: "건", aggregation: "sum", formula: "전체카드 이용건수 월 합계(전수화)", limitation: "2명 미만 레코드는 원자료에서 익명처리됨", triggers: ["결제건수", "카드건수", "이용건수"] },
  ],
};
...
export const KCB_CREDIT_LAYER: Omit<LayerDescriptor, "months"> = {
  ...
  metrics: [
    { key: "avg_income", label: "평균소득", unit: "만원/월", aggregation: "weightedAvg", weightKey: "pop_total", formula: "연령구간 월소득 평균의 인구가중 평균", limitation: "KCB 추정 거주자 소득, 세전 개인 기준", triggers: ["평균소득", "소득수준", "월소득", "소득"] },
    ...
    { key: "loan_ratio", label: "대출보유율", unit: "%", aggregation: "weightedAvg", weightKey: "pop_total", formula: "대출 보유자 수 ÷ 인구 × 100", limitation: "보유 여부 기준(잔액 아님)", triggers: ["대출", "대출보유", "부채"] },
