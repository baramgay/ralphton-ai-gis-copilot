...
const CASES = [
  ...
  // 전체 1위가 411.3만원/월이므로 400만원 이상은 소수만 남는다 — 24행 표시 상한에
  // 가려지지 않는, 필터가 실제로 걸렸다는 증거다. (300만원으로 잡으면 통과 동이 24개를
  // 넘어 상한에 가려져 증거가 되지 못한다 — 실제로 그렇게 오판했다.)
  ["소득 400만원 이상인 동", (r) => r.caveat === "" && r.rows > 0 && r.rows < 24, "값 조건 반영(소득)"],
