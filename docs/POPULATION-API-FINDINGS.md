...
다섯 단계를 한 묶음으로 반영했다. 실 API로 2개 읍면동을 태워 13개월 시계열이 채워지는 것을
확인했다.

```
인구 live: 경남 2개 읍면동의 인구·세대 13개월 시계열을 실데이터로 교체했습니다.
문산읍 실측 7487 7480 7468 7478 7479 7470 7466 7461 7446 7396 7392 7392 7379
문산읍 합성 50000 49842 49684 49527 49370 49214 49058 48903 48749 48594 48441 48287 48135
```

합성 스냅샷은 모든 읍면동을 5만 명 언저리에서 출발시켰다. 문산읍은 실제로 7,487명이다
— 인구 순위가 실제와 다를 수밖에 없었다.

**전부 아니면 전무로 간다.** 한 읍면동의 한 달이라도 빠지면 기준 스냅샷을 그대로 둔다.
일부만 실데이터로 바뀌면 지역 간 순위가 실측과 합성을 섞어 비교하게 되고, 그것은 사용자가
알아챌 수 없는 조용히 틀린 답이다.

`sync` 라우트에 `maxDuration = 300`을 뒀다 — 1,220회가 기본 60초를 넘고, 중간에 끊기면
위 규칙에 따라 통째로 버려진다.
