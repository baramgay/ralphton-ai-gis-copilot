...
`POST /api/ai/parse`는 입력 길이를 제한하고 Zod 스키마로 본문을 검증한다. 서버는 가성비 모델 `qwen3.6-flash`에 JSON 모드와 비사고 모드를 요청하고, 파싱 또는 Zod 검증 실패 시 한 번 재시도한다. 이후 `qwen3.7-plus`로 한 번 fallback한다. 모든 실패 또는 키 없음에서는 같은 `AnalysisIntent`를 내는 결정론적 한국어 규칙 파서를 사용하고 데모 모드를 표시한다.
